...
import { userVoltageDummy, withUserDetailFallback } from '../../data/analyticsDummy'
...
export default function UserVoltageImbalance() {
  ...
  const { data, loading, reload } = useFetch(async () => {
    if (!selectedDeviceId) {
      return withUserDetailFallback(null, userVoltageDummy)
    }
  ...
  return (
    <AnalyticsDetailPage
      ...
      value={data?.value ?? userVoltageDummy.value}
      anomalyType="Overvoltage"
      anomalyRows={data?.anomalyRows ?? userVoltageDummy.anomalyRows}
      predictedTitle="Predicted Voltage"
      predictedType="bar"
      predictedData={data?.predictedData ?? userVoltageDummy.predictedData}
      predictedColor="#3B82F6"
      overTimeTitle="Voltage Over Time"
      overTimeType="bar"
      overTimeData={data?.overTimeData ?? userVoltageDummy.overTimeData}
