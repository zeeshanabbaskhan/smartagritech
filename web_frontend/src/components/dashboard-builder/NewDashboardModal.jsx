...
export default function NewDashboardModal({ open, onClose, onCreate, devices = [] }) {
  const [templateId, setTemplateId] = useState('blank')
  const [name, setName] = useState('')
  const [targetDeviceId, setTargetDeviceId] = useState('')

  function handleCreate() {
    onCreate(templateId, name.trim(), targetDeviceId || null)
    setName('')
    setTemplateId('blank')
    setTargetDeviceId('')
    onClose()
            ...
            {DASHBOARD_TEMPLATES.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[10px] text-surface-400 mt-1.5 font-semibold uppercase tracking-wide">
