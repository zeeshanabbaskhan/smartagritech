...
function extractDeviceMap(nodes, map = new Map()) {
  for (const n of nodes || []) {
    const key = `${n.type}::${n.name}`
    if (n.deviceIds?.length) map.set(key, n.deviceIds)
    if (n.children?.length) extractDeviceMap(n.children, map)
  }
  return map
}

function applyDeviceMap(nodes, map) {
  return (nodes || []).map((n) => ({
    ...n,
    deviceIds: map.get(`${n.type}::${n.name}`) || n.deviceIds || [],
    children: applyDeviceMap(n.children, map),
  }))
}

function buildTreeFromFormFields(fields, orgName, previousTree = []) {
  const order = [
    'Organization', 'Campus', 'Site', 'Building', 'Block', 'Wing',
    'Floor', 'Zone', 'Room', 'Department', 'Section', 'Asset', 'Device', 'Sensor'
  ]
  
  let root = { id: 'root-org', name: orgName || 'My Organization', type: 'Organization', children: [], deviceIds: [] }
  ...
  for (let i = 1; i < order.length; i++) {
    ...
    for (const parent of currentLevelNodes) {
      ...
      for (const name of names) {
        const child = { id: `node-${type}-${Math.random().toString(36).slice(2, 7)}`, name, type, children: [], deviceIds: [] }
  ...
  return applyDeviceMap([root], extractDeviceMap(previousTree))
}
...
export default function HierarchyEditor({ buildings, onChange, orgName }) {
  ...
  const handleFieldChange = (type, val) => {
    ...
    setFields(nextFields)
    const nextTree = buildTreeFromFormFields(nextFields, orgName, tree)
    onChange(nextTree)
  }
